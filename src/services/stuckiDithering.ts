// services/stuckiDithering.ts

export function stuckiDithering(imageData: ImageData, errorDiffusionFactor: number): ImageData {
    const { data, width, height } = imageData;
    const newImageData = new Uint8ClampedArray(data);

    // The error diffusion matrix for Stucki Dithering
    const diffusionMatrix = [
        { x: 1, y: 0, factor: 8 / 42 * errorDiffusionFactor },
        { x: 2, y: 0, factor: 4 / 42 * errorDiffusionFactor },
        { x: -2, y: 1, factor: 2 / 42 * errorDiffusionFactor },
        { x: -1, y: 1, factor: 4 / 42 * errorDiffusionFactor },
        { x: 0, y: 1, factor: 8 / 42 * errorDiffusionFactor },
        { x: 1, y: 1, factor: 4 / 42 * errorDiffusionFactor },
        { x: 2, y: 1, factor: 2 / 42 * errorDiffusionFactor },
        { x: -2, y: 2, factor: 1 / 42 * errorDiffusionFactor },
        { x: -1, y: 2, factor: 2 / 42 * errorDiffusionFactor },
        { x: 0, y: 2, factor: 4 / 42 * errorDiffusionFactor },
        { x: 1, y: 2, factor: 2 / 42 * errorDiffusionFactor },
        { x: 2, y: 2, factor: 1 / 42 * errorDiffusionFactor }
    ];
