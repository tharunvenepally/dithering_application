// services/atkinsonDithering.ts

export function atkinsonDithering(imageData: ImageData, errorDiffusionFactor: number): ImageData {
    const { data, width, height } = imageData;
    const newImageData = new Uint8ClampedArray(data);

    // The error diffusion matrix for Atkinson Dithering
    const diffusionMatrix = [
        { x: 1, y: 0, factor: 1 / 8 * errorDiffusionFactor },
        { x: 2, y: 0, factor: 1 / 8 * errorDiffusionFactor },
        { x: -1, y: 1, factor: 1 / 8 * errorDiffusionFactor },
        { x: 0, y: 1, factor: 1 / 8 * errorDiffusionFactor },
        { x: 1, y: 1, factor: 1 / 8 * errorDiffusionFactor },
        { x: 0, y: 2, factor: 1 / 8 * errorDiffusionFactor }
    ];
